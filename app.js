...
// it twice causes every request to hit /rest/v1/rest/v1/... which returns 404.
const _cleanUrl = SUPABASE_URL.replace(/\/rest\/v1\/?$/, '').replace(/\/$/, '');

let db = null, dbReady = false;

// ─── ERROR LOGGER ─────────────────────────────────────────────
// Central error handler so DB/network failures are always visible
// in the UI (toast + console) instead of silently swallowed.
// In production this can also POST to an error tracking service.
...
async function syncFromSupabase() {
  if (!dbReady) return;
  try {
    const { data: dbP, error: pErr } = await db.from('products').select('*').eq('business_id',businessId).order('id'); if(pErr) logError('Load products',pErr);
    if (dbP && dbP.length) { products = dbP.map(p=>({id:p.id,cat:p.category,name:p.name,price:p.price,icon:p.icon||'🔧'})); renderProducts(); renderSetup(); }
    const today = new Date().toISOString().split('T')[0];
    const { data: dbT, error: tErr } = await db.from('transactions').select('*').eq('business_id',businessId).order('created_at',{ascending:false}).limit(500); if(tErr) logError('Load transactions',tErr);
    if (dbT && dbT.length) { transactions = dbT.map(t=>({id:t.id,time:new Date(t.created_at).toLocaleTimeString('en-KE',{hour:'2-digit',minute:'2-digit'}),timestamp:t.created_at,items:t.items,total:t.total,payment:t.payment_method,splits:t.splits||null,staff:t.staff_name,plate:t.plate_number||'',customerPhone:t.customer_phone||'',customerName:t.customer_name||'',isFreeWash:t.is_free_wash||false,isFullyFree:t.is_fully_free!==undefined?t.is_fully_free:(t.is_free_wash&&t.total===0)})); save(); }
    const { data: dbC, error: cErr } = await db.from('customers').select('*').eq('business_id',businessId); if(cErr) logError('Load customers',cErr);
    if (dbC && dbC.length) { customersByPlate = {}; dbC.forEach(c => customersByPlate[c.plate] = {name:c.name,phone:c.phone,visits:c.visits,freeWashAvailable:c.free_wash_available}); save(); }
    const { data: dbBiz, error: bErr } = await db.from('businesses').select('*').eq('id',businessId).maybeSingle(); if(bErr) logError('Load business settings',bErr);
    if (dbBiz) { businessName = dbBiz.name || businessName; businessLogo = dbBiz.logo || businessLogo; businessVertical = dbBiz.vertical || businessVertical; businessNameColor = dbBiz.name_color || businessNameColor; save(); renderNavBranding(); }
...
async function saveCustomerToDb(plate, c) {
  if (!dbReady) return;
  try { const {error:e3} = await db.from('customers').upsert({identifier_value:plate,business_id:businessId,name:c.name,phone:c.phone,visits:c.visits,free_wash_available:c.freeWashAvailable}); if(e3) logError('Save customer',e3); } catch(e) { logError('Save customer',e); }
}

// ─── VERTICAL REGISTRY ────────────────────────────────────────
// Each entry describes how this vertical tracks a customer/order within
// the system. Changing the active vertical (via Setup) swaps these labels
// and placeholder values across the entire POS, reports, and alerts -
// no logic changes needed per vertical since all field names stay the same
// ('plate' internally); only the user-facing text changes.
const VERTICAL_REGISTRY = {
  'car_wash': {
    id:'car_wash', emoji:'🚗',
    identifierLabel:'Plate number',
    identifierPlaceholder:'e.g. KBZ 123A',
    identifierInputStyle:'text-transform:uppercase',
    identifierRequiredMsg:'Plate number is required',
    identifierSearchHint:'Vehicle',
    loyaltyNoun:'vehicle',        // "Vehicle KBZ 123A has a free wash"
    loyaltyEntity:'wash',         // "Free wash earned"
    serviceNoun:'wash',           // used in loyalty alert messages
    identifierRequired:true,
    approvalAutoLimit:500
  },
  'salon': {
    id:'salon', emoji:'✂️',
    identifierLabel:'Customer / Member ID',
    identifierPlaceholder:'e.g. SLN-001 or name',
    identifierInputStyle:'',
    identifierRequiredMsg:'Customer or Member ID is required',
    identifierSearchHint:'Customer',
    loyaltyNoun:'customer',
    loyaltyEntity:'service',
    serviceNoun:'service',
    identifierRequired:false,
    approvalAutoLimit:300
  },
  'restaurant': {
    id:'restaurant', emoji:'🍽️',
    identifierLabel:'Table / Order number',
    identifierPlaceholder:'e.g. T-12 or TAK-001',
    identifierInputStyle:'',
    identifierRequiredMsg:'Table or order number is required',
    identifierSearchHint:'Table',
    loyaltyNoun:'customer',
    loyaltyEntity:'meal',
    serviceNoun:'meal',
    identifierRequired:false,
  ...
  'retail': {
    id:'retail', emoji:'🏪',
    identifierLabel:'Customer / Loyalty ID',
    identifierPlaceholder:'e.g. RET-0042',
    identifierInputStyle:'',
    identifierRequiredMsg:'Customer ID is required',
    identifierSearchHint:'Customer',
    loyaltyNoun:'customer',
    loyaltyEntity:'purchase',
    serviceNoun:'purchase',
    identifierRequired:false,
    approvalAutoLimit:1000
  },
  'pharmacy': {
    id:'pharmacy', emoji:'💊',
    identifierLabel:'Patient / Prescription ID',
    identifierPlaceholder:'e.g. PAT-2025-001',
    identifierInputStyle:'',
    identifierRequiredMsg:'Patient ID is required',
    identifierSearchHint:'Patient',
    loyaltyNoun:'patient',
    loyaltyEntity:'prescription',
    serviceNoun:'prescription',
    identifierRequired:false,
    approvalAutoLimit:500
  },
  'laundry': {
    id:'laundry', emoji:'👕',
    identifierLabel:'Collection / Order tag',
    identifierPlaceholder:'e.g. LND-0099',
    identifierInputStyle:'',
    identifierRequiredMsg:'Collection tag is required',
    identifierSearchHint:'Order',
    loyaltyNoun:'customer',
    loyaltyEntity:'order',
    serviceNoun:'order',
    identifierRequired:false,
...
// Maps the vertical select dropdown value (emoji string) to a registry key
const VERTICAL_EMOJI_TO_KEY = {
  '🚗':'car_wash', '✂️':'salon', '🍽️':'restaurant',
  '🏪':'retail', '💊':'pharmacy', '👕':'laundry'
};
...
// Apply the active vertical's labels across the POS form in real-time.
// Called whenever the vertical is changed in Setup or on app entry.
function applyVerticalLabels() {
  const vc = getVerticalConfig();
  // POS identifier field label
  ...
  // Update column headers in Sales and Credit reports
  ...
  // Custom report field label for plate
  if (typeof CUSTOM_REPORT_FIELDS !== 'undefined') {
    const plateField = CUSTOM_REPORT_FIELDS.find(f => f.key === 'plate');
    if (plateField) plateField.label = vc.identifierLabel;
  }
}


// ─── RBAC: USERS ────────────────────────────────────────────────
// Demo users. In production this table lives in Supabase with hashed
// passwords; this is a working stub so the maker-checker flow is real today.
// ─── ROLE PERMISSIONS ─────────────────────────────────────────
const ROLE_PERMISSIONS = {
  maker:   ['pos','log','recon','alerts','setup','credit','myreturns'],
  checker: ['pos','log','recon','approvals','alerts','setup','credit']
};
let currentUser = null; // { id, username, name, role, businessId }

// ═══════════════════════════════════════════════════════════════
// AUTH — PIN-BASED + SECURITY QUESTION RECOVERY
// No email required, fully self-contained, no SMS gateway needed.
// ═══════════════════════════════════════════════════════════════

// ── Security questions offered at registration ─────────────────
...
// ── 1. PIN STRETCHING ──────────────────────────────────────────
// Converts a short 4-digit PIN into a longer string that satisfies
// Supabase Auth's minimum password length requirement.
// The stretch formula includes a salt suffix so even if two users
// share the same PIN (e.g. "1234"), their Supabase passwords differ
// because their usernames differ.
// IMPORTANT: Never change this formula after going live — existing
// passwords cannot be recovered if the formula changes.
function stretchPin(pin, username) {
  return pin + '_' + username.trim().toLowerCase() + '_mulika_secure_2025';
}

// ── 2. Username → masked email ─────────────────────────────────
// Supabase Auth requires an email format. We mask the username into
// one silently so users only ever type their plain username.
function usernameToEmail(username) {
  return username.trim().toLowerCase().replace(/\s+/g, '.') + '@mulika.internal';
}

// ── 3. Answer hashing ─────────────────────────────────────────
// Security question answers are hashed with SHA-256 before storage
// so the raw answer is never persisted in the database — even a
// database dump cannot reveal what someone answered.
async function hashAnswer(answer) {
  const normalized = answer.trim().toLowerCase(); // case-insensitive, trimmed
...
async function attemptLogin() {
  ...
  if (!db) {
    errEl.textContent = 'Not connected to server. Check your internet.';
    errEl.classList.add('show'); return;
  }
  ...
  try {
    ...
    if (authError) {
      errEl.textContent = 'Incorrect username or PIN';
      errEl.classList.add('show');
      logLoginEvent(username, false);
      return;
    }

    // Fetch role + business from profiles table
    ...
    if (profileError || !profile) {
      errEl.textContent = 'Account found but profile missing. Contact your administrator.';
      errEl.classList.add('show');
...
// ── 5. PIN VISIBILITY TOGGLE (shared by login + recovery) ──────
...
// ── 6. RECOVERY — Step 1: Look up username ────────────────────
let _recoveryProfile = null; // holds profile row during recovery flow
...
function showRecovery() {
  ...
  document.getElementById('recovery-answer') && (document.getElementById('recovery-answer').value = '');
  document.getElementById('recovery-new-pin') && (document.getElementById('recovery-new-pin').value = '');
  document.getElementById('recovery-confirm-pin') && (document.getElementById('recovery-confirm-pin').value = '');
...
async function fetchSecurityQuestion() {
  ...
  try {
    // We need the business_id to look up the profile.
    // It is stored in localStorage from when the app was first set up.
    const storedBizId = localStorage.getItem('ib_business_id') || businessId;
    if (!storedBizId) {
      errEl.textContent = 'Business not configured on this device. Log in from the device you normally use.';
      errEl.classList.add('show'); return;
    }

    // Use the anon-accessible RPC — the user is not authenticated yet
    const { data, error } = await db.rpc('get_profile_for_recovery', {
      p_username: username.toLowerCase().trim(),
      p_business_id: storedBizId
    });

    if (error) { logError('Recovery lookup', error); return; }
    const profile = data && data[0];
    if (!profile || !profile.security_question) {
      errEl.textContent = 'Username not found. Check your spelling.';
      errEl.classList.add('show'); return;
    }

    // Cache the profile for steps 2 and 3 — nothing sensitive
    // is stored here that isn't already in the DB response
    _recoveryProfile = {
      user_id: profile.user_id,
      display_name: profile.display_name,
      role: profile.role,
      security_question: profile.security_question,
      security_answer_hash: profile.security_answer_hash,
...
// ── 7. RECOVERY — Step 2: Verify security answer ──────────────
async function verifySecurityAnswer() {
  ...
  if (!answer.trim()) {
    errEl.textContent = 'Please type your answer';
    errEl.classList.add('show'); return;
  }
  if (!_recoveryProfile) {
  ...
  try {
    ...
    // Answer correct — unlock step 3
    document.getElementById('recovery-step-2').style.display = 'none';
    document.getElementById('recovery-step-3').style.display = 'block';
    setRecoveryStep(3);
    setTimeout(() => document.getElementById('recovery-new-pin').focus(), 100);
  } catch(e) {
...
// ── 8. RECOVERY — Step 3: Save new PIN and log in ────────────
async function submitNewPin() {
  ...
  if (!newPin || !confirmPin) {
    errEl.textContent = 'Please enter and confirm your new PIN';
    errEl.classList.add('show'); return;
  }
  if (!/^\d{4}$/.test(newPin)) {
    errEl.textContent = 'PIN must be exactly 4 digits';
  ...
  if (newPin !== confirmPin) {
    ...
    errEl.classList.add('show');
    document.getElementById('recovery-confirm-pin').value = '';
    return;
  }
  if (!_recoveryProfile) {
    errEl.textContent = 'Session expired. Please start again.';
  ...
  try {
    ...
    // Pass the stored answer hash to the RPC so it can re-verify
    // server-side — the server never trusts the client alone
    const { data: rpcResult, error: rpcError } = await db.rpc('reset_user_pin', {
      p_user_id: _recoveryProfile.user_id,
      p_new_stretched_password: newStretched,
      p_answer_hash: _recoveryProfile.security_answer_hash
    });

    if (rpcError || (rpcResult && !rpcResult.success)) {
      const msg = rpcError?.message || (rpcResult && rpcResult.error) || 'PIN reset failed';
      logError('PIN reset', { message: msg });
      errEl.textContent = msg;
      errEl.classList.add('show'); return;
    }

    showToast('✅ PIN updated. Logging you in…');

    // Auto-login with the new PIN immediately
    const maskedEmail = usernameToEmail(_recoveryProfile.username);
    const { data: authData, error: authError } = await db.auth.signInWithPassword({
      email: maskedEmail,
      password: newStretched
    });

    if (authError) {
      // PIN was saved — just ask them to log in manually
      showToast('PIN saved. Please log in with your new PIN.');
      hideRecovery();
      return;
    }
    ...
    currentUser = {
      id: authData.user.id,
      username: _recoveryProfile.username,
      name: _recoveryProfile.display_name,
      role: _recoveryProfile.role,
      businessId: _recoveryProfile.business_id
    };
    _recoveryProfile = null;
    hideRecovery();
...
// ── 10. SIGN UP (new business registration) ───────────────────
function showSignUp() {
  document.getElementById('login-card').style.display = 'none';
